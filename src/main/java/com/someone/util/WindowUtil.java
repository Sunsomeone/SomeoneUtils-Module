...
public class WindowUtil {
    ...
    public static void init() {
        ...
        final View decorView = GlobalUtilSetting.getActivity().getWindow().getDecorView();
        ...
        if (statusBarHeight <= 0 || navigationBarHeight <= 0) {
            ...
            // 状态栏高度反射获取
            if (statusBarHeight <= 0) {
                @SuppressLint({"InternalInsetResource", "DiscouragedApi"}) int resourceId = Resources.getSystem().getIdentifier(
        ...
        if (statusBarHeight <= 0) {
            statusBarHeight = ViewUtil.toPx(MIN_STATUS_BAR_DP);
        }
        if (navigationBarHeight <= 0) {
            navigationBarHeight = ViewUtil.toPx(MIN_NAV_BAR_DP);
        }
    }
